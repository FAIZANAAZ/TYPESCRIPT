...
let condition2 = true;
while (condition2) {
    let ubdate_list = await inquirer.prompt([{
            name: "edit",
            type: "confirm",
    ...
    if (ubdate_list.edit === true) {
        let choies = await inquirer.prompt([
            {
                name: "ubdating",
                type: "list",
        ...
        if (choies.ubdating === "Edit") {
            let step = main_data.map((item, index) => {
                return {
            ...
            switch (item.delete) {
                case "Name":
                    main_data[select.it].employ = item.new;
                    break;
                case "Id":
                    main_data[select.it].id = Number(item.new);
                    break;
                case "Sellery":
                    main_data[select.it].sellery = Number(item.new);
                    break;
            }
